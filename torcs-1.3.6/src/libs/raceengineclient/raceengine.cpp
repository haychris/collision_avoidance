...
/////////////////////////// by Chenyi, output 1 image per 0.1 second
	...
	tCarElt* obstacleCarLeft;
	tCarElt* obstacleCarRight;

	tCarElt* curCar;
	for (i = 0; i < s->_ncars; i++) {
		if (s->cars[i]->info.startRank == 0) {
			obstacleCarRight = s->cars[i];
		}
		if (s->cars[i]->info.startRank == 1) {
			obstacleCarLeft = s->cars[i];
		} 
		if (s->cars[i]->info.startRank == 2) {
			obstacleCarAhead = s->cars[i];
		} 
		if (s->cars[i]->info.startRank == 3) {
			curCar = s->cars[i];
		}  
	}
	...
	if (s->_ncars == NUM_TRAIN_CARS && !printed_initial_speeds && s->currentTime > 0.001) {
		...
		dataFile << "obstacleCarRight starting_speed: " << obstacleCarRight->pub.speed << "\n";
		dataFile << "curCar starting_speed: " << curCar->pub.speed << "\n";

		dataFile << "obstacleCarAhead starting_distance: " << obstacleCarAhead->_distFromStartLine - curCar->_distFromStartLine << "\n";
		dataFile << "obstacleCarLeft starting_distance: " << obstacleCarLeft->_distFromStartLine - curCar->_distFromStartLine << "\n";
	...
	if (s->_ncars == NUM_TRAIN_CARS && !went_offroad) {
    ...
    if(restartRequested){  
      	if (s->_ncars == NUM_TRAIN_CARS) {
      		dataFile << "obstacleCarAhead final_damage: " << obstacleCarAhead->priv.dammage << "\n"; 
  			dataFile << "obstacleCarLeft final_damage: " << obstacleCarLeft->priv.dammage << "\n"; 
      		dataFile << "obstacleCarRight final_damage: " << obstacleCarRight->priv.dammage << "\n"; 
  			dataFile << "curCar final_damage: " << curCar->priv.dammage << "\n";

  			dataFile << "obstacleCarAhead final_speed: " << obstacleCarAhead->pub.speed << "\n";
			dataFile << "obstacleCarLeft final_speed: " << obstacleCarLeft->pub.speed << "\n";
  			dataFile << "obstacleCarRight final_speed: " << obstacleCarRight->pub.speed << "\n";
			dataFile << "curCar final_speed: " << curCar->pub.speed << "\n";

			double final_distance_obstacleCarAhead = obstacleCarAhead->_trkPos.seg->lgfromstart + getDistToSegStart(obstacleCarAhead) - curCar->_distFromStartLine;
    	    if (final_distance_obstacleCarAhead > ReInfo->track->length/2.0f) {
    	        final_distance_obstacleCarAhead -= ReInfo->track->length;
	        } else if (final_distance_obstacleCarAhead < -ReInfo->track->length/2.0f) {
	            final_distance_obstacleCarAhead += ReInfo->track->length;
	        }

	        double final_distance_obstacleCarLeft = obstacleCarLeft->_trkPos.seg->lgfromstart + getDistToSegStart(obstacleCarLeft) - curCar->_distFromStartLine;
    	    if (final_distance_obstacleCarLeft > ReInfo->track->length/2.0f) {
    	        final_distance_obstacleCarLeft -= ReInfo->track->length;
	        } else if (final_distance_obstacleCarLeft < -ReInfo->track->length/2.0f) {
	            final_distance_obstacleCarLeft += ReInfo->track->length;
	        }

	        double final_distance_obstacleCarRight = obstacleCarRight->_trkPos.seg->lgfromstart + getDistToSegStart(obstacleCarRight) - curCar->_distFromStartLine;
    	    if (final_distance_obstacleCarRight > ReInfo->track->length/2.0f) {
    	        final_distance_obstacleCarRight -= ReInfo->track->length;
	        } else if (final_distance_obstacleCarRight < -ReInfo->track->length/2.0f) {
	            final_distance_obstacleCarRight += ReInfo->track->length;
	        }
	        // final_distance *= curCar->race.pos -  obstacleCarAhead->race.pos;
			dataFile << "final_distance_obstacleCarAhead: " << final_distance_obstacleCarAhead << "\n";
  			dataFile << "final_distance_obstacleCarLeft: " << final_distance_obstacleCarLeft << "\n";
			dataFile << "final_distance_obstacleCarRight: " << final_distance_obstacleCarRight << "\n";
