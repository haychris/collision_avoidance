...
initStartingGrid(void)
{
	...
	tdble curCarSpeed = rand() % 130; //150
	tdble obstacleCarAheadSpeed = rand() % 130; //150
	tdble obstacleCarLeftSpeed = rand() % 130; //150
	...
	tdble desiredOffsetLeft = rand() % MAX_DESIRED_OFFSET + MIN_DESIRED_OFFSET;
	tdble desiredOffsetRight = rand() % MAX_DESIRED_OFFSET + MIN_DESIRED_OFFSET;
	tdble maxOffset = std::max(desiredOffsetAhead, std::max(desiredOffsetLeft, desiredOffsetRight));
